// Cursor trail shader from KroneCorylus/ghostty-shader-playground:
...
// 4. Fading the tail away from the current cursor
// 5. Enhancing color saturation for visual impact
...
// Only smear cursor jumps larger than this many pixels. This avoids the
// "lagging cursor" feeling while typing through normal character movement.
const float MIN_JUMP_DISTANCE = 80.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Skip further work when animation is complete, the cursor only moved a
    // small amount, such as while typing ordinary text.
    if (baseProgress >= 1.0 || length(iCurrentCursor.xy - iPreviousCursor.xy) < MIN_JUMP_DISTANCE) {
    ...
    // Fade the trail away from the current cursor. This keeps the front of the
    // smear vivid while the tail dissolves toward the previous cursor position.
